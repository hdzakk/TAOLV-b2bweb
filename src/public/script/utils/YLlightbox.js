...
/*=====lightbox方法=====*/
	function imgPreview(){
		...
		$('.YLlightbox a[rel="YLlightbox"]').click(function(e){
				...
				e.stopPropagation();
				//显示图片
				$('#YLimg').attr('src',$(this).attr('href')).css({'min-width':'300px','max-height':'600px','min-height':'200px'});
				$('#YLimgwrap').show();
				//获取图片尺寸
		...
		$('#YLimgcon .prev').click(function(){
				$('#YLimg').width("").height("").attr('src',imgSrcArray[a_flag_index -1]).css({'max-height':'600px','min-height':'200px'});
				//获取图片尺寸
				var tranImg=new Image();
				tranImg.src=$('#YLimg').attr('src');
				base_w=tranImg.width;		//图片原始宽度
