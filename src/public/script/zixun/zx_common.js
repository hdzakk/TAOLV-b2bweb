define(['script/utils/angular','script/index/load_init','script/utils/req'],function(angular, LOAD ,req){
	...
	.filter('typeUrl', function() {
		return function(d) {
			...
			if (d.templateCode == "WZ") {
				href = '/page/zixun/news_details.html?typeSn='+d.typeSn+'&sn='+d.sn+'&code='+url.code +'&lm=' +url.lm;
			}else if (d.templateCode == "SP") {
				href = '/page/zixun/vid_details.html?typeSn='+d.typeSn+'&sn='+d.sn+'&code='+url.code +'&lm=' +url.lm;//+'&playUrl='+d.url;
			}else{
				href = d.jumpUrl;
			}
			return href;
	...
	.filter('toStart', function() {
		return function(perDate) {
			if(perDate=='NODE_ORDER_SALE_master_000'){
				return '已下单|待审核';
			}else if (perDate==11) {
				return '审核不通过|订单作废';
			}else if (perDate==12) {
